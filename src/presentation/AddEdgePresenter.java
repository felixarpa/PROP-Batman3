...
public class AddEdgePresenter extends MainAdminPresenter {
    ...
    public void addEdge() {
        ...
        try {
            ...
            ((ManageEdgeView)actualView).showErrorMessage("Please select nodes to add an edge between them");
            return;
        }
        if (type1 != ProjectConstants.PAPER_TYPE && type2 != ProjectConstants.PAPER_TYPE) {
            ((ManageEdgeView)actualView).showErrorMessage("Can't make an edge between two nodes not directly connected (one of them must be paper)");
            return;
        }
        try {
            adminController.addNewEdge(id1, type1, id2, type2);
            ((ManageEdgeView)actualView).showSuccessMessage("Edge added successfully");
        } catch (ExistingEdge existingEdge) {
            ((ManageEdgeView)actualView).showErrorMessage("An edge already exists between the nodes");
